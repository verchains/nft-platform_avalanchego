...
AVALANCHE_BINARY_PATH=${AVALANCHE_BINARY_PATH:-$avalanchego_path}

# Build with rocksdb allowed only if the environment variable ROCKSDBALLOWED is set
if [ -z ${ROCKSDBALLOWED+x} ]; then
    echo "Building AvalancheGo..."
    go build -ldflags "-X github.com/ava-labs/avalanchego/version.GitCommit=$git_commit $static_ld_flags" -o "$AVALANCHE_BINARY_PATH" "$AVALANCHE_PATH/main/"*.go
else
    echo "Building AvalancheGo with rocksdb enabled..."
    go build -tags rocksdballowed -ldflags "-X github.com/ava-labs/avalanchego/version.GitCommit=$git_commit $static_ld_flags" -o "$AVALANCHE_BINARY_PATH" "$AVALANCHE_PATH/main/"*.go
fi
